...
import {
  Layers, ClipboardList, MessageCircle, Bell, Star, Users, BookOpen, Briefcase, Lightbulb, Wallet2, LifeBuoy, LogOut, Inbox
} from "lucide-react";

export default withRoleProtection(PainelVendedor, { allowed: ["vendedor"] });
...
function PainelVendedor() {
  ...
  useEffect(() => {
    async function fetchMetrics() {
      ...
      try {
        const maquinasQuery = query(collection(db, "maquinas"), where("userId", "==", user.uid));
        const maquinasSnap = await getCountFromServer(maquinasQuery);
        const propostasQuery = query(collection(db, "propostas"), where("userId", "==", user.uid));
        const propostasSnap = await getCountFromServer(propostasQuery);
        const servicosQuery = query(collection(db, "servicos"), where("userId", "==", user.uid));
        const servicosSnap = await getCountFromServer(servicosQuery);
        const leadsQuery = query(collection(db, "leads"), where("vendedorId", "==", user.uid));
        const leadsSnap = await getCountFromServer(leadsQuery);
        ...
        setMetrics({
          maquinas: maquinasSnap.data().count,
          ...
          servicos: servicosSnap.data().count,
          parceiros: 0, // Preencha se for trazer parceiros cadastrados
        });
      } catch (err) {
        console.error("Erro ao buscar métricas do vendedor:", err);
      } finally {
    ...
    <main style={{
      ...
      <section style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <MetricBadge icon={<ClipboardList size={15} />} value={loadingMetrics ? "..." : metrics.propostas} label="propostas" color="#2563eb" />
        ...
        >
          ...
          <Tile href="/minhas-maquinas" color="#FB8500" bg="#fff7ed" icon={<Layers size={36} />} title="Minhas Máquinas" desc="Gerencie suas máquinas publicadas e status." badge={metrics.maquinas} />
          ...
          <Tile href="/minhas-propostas" color="#2563eb" bg="#f3f7ff" icon={<ClipboardList size={36} />} title="Minhas Propostas" desc="Acompanhe propostas enviadas e recebidas." badge={metrics.propostas} />
          <Tile href="/mensagens" color="#FB8500" bg="#fff7ed" icon={<MessageCircle size={36} />} title="Mensagens" desc="Converse com clientes e negocie direto." badge={metrics.mensagens} />
          <Tile href="/notificacoes" color="#FB8500" bg="#fff7ed" icon={<Bell size={36} />} title="Notificações" desc="Alertas e novidades da plataforma." badge={metrics.notificacoes} />
          <Tile href="/avaliacoes" color="#FB8500" bg="#fff7ed" icon={<Star size={36} />} title="Avaliações Recebidas" desc="Confira feedbacks de clientes." />
          <Tile href="/parceiros" color="#FB8500" bg="#fff7ed" icon={<Users size={36} />} title="Parceiros" desc="Conheça empresas e parceiros." />
          <Tile href="/blog" color="#FB8500" bg="#fff7ed" icon={<BookOpen size={36} />} title="Blog" desc="Acesse conteúdos e notícias do setor." />
      ...
      <footer style={{
        ...
        © {new Date().getFullYear()} Pedraum Brasil · Painel do Vendedor.
      </footer>
    </main>
  );
}
