...
export default function PainelUnificado() {
  ...
  // helper: tenta vários campos possíveis
  function qByAnyField(colName: string, uid: string, fieldNames: string[], extra?: (ref: any) => any) {
  ...
  async function fetchMetrics(uid: string) {
    ...
    try {
      ...
      // notificações
      const notifTotalQ  = qByAnyField("notificacoes", uid, ["usuarioId", "userId"]);
      const notifUnreadQ = qByAnyField("notificacoes", uid, ["usuarioId", "userId"], 
        (base) => query(base, where("lida" as any, "==", false))
      );

      // mensagens
      const msgTotalQ  = qByAnyField("mensagens", uid, ["destinatarioId", "toUserId", "userId"]);
      const msgUnreadQ = qByAnyField("mensagens", uid, ["destinatarioId", "toUserId", "userId"], 
        (base) => query(base, where("lido" as any, "==", false))
      );
      ...
      const [
        maquinas, produtos, servicos, leads, demandas, favoritos, propostas, pedidos, sugestoes, avaliacoes,
        notifUnread, notifTotal, msgUnread, msgTotal,
        oppSent, oppViewed, oppUnlocked
      ] = await Promise.all([
        count(maquinasQ), count(produtosQ), count(servicosQ), count(leadsQ), count(demandasQ),
        count(favQ), count(propQ), count(pedidosQ), count(sugestQ), count(avalQ),
        count(notifUnreadQ), count(notifTotalQ), count(msgUnreadQ), count(msgTotalQ),
        count(oppSentQ), count(oppViewedQ), count(oppUnlockedQ)
      ]);

      const notificacoes = notifUnread > 0 ? notifUnread : notifTotal;
      const mensagens    = msgUnread   > 0 ? msgUnread   : msgTotal;
      ...
      <section style={{
        ...
        <div style={{
          ...
          <button onClick={handleLogout} disabled={loading} style={{
            background: "#fff3ea", border: "1.5px solid #ffb680", borderRadius: 22,
            boxShadow: "0 8px 36px #0001", padding: "32px 8px",
            display: "flex", flexDirection: "column", alignItems: "center", gap: 10,
            color: "#E85D04", fontWeight: 700, fontSize: "1.05rem", minHeight: 185,
          }}>
...
function MetricBadge({ icon, value, label, color }: { icon: React.ReactNode; value: string | number; label: string; color: string }) {
  return (
    <div style={{
      ...
      background: "#f6f9fa", borderRadius: 13, padding: "6px 14px",
      fontSize: ".97rem", color: "#023047", fontWeight: 700,
    ...
    >
      ...
      {open && (
        <div style={{
          ...
          fontSize: ".9rem", width: 240, zIndex: 10, boxShadow: "0 6px 20px #0002"
        }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <strong>O que é isto?</strong>
            <X size={14} onClick={() => setOpen(false)} style={{ cursor: "pointer" }} />
          </div>
          <p>{text}</p>
        </div>
      )}
